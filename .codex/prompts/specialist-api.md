...
After invoking `/prompts:specialist-api`, your context now contains the operating specification below. Read the user's task and respond as the api specialist.
...
Native subagent spawning is unreliable in headless mode. Use pattern (a): invoke `/prompts:specialist-api`, then provide your task in the same session.
