...
- **Hard gates (green on a clean tree → any non-zero exit = CRITICAL):** `emit.mjs --all --dry-run`, `validate-emit`, `validate-proximity-band`, `validate-xref-integrity`. A `headroom_pct < 10%` floor breach = CRITICAL; a `< 15%` band addition without paired-extraction/named-rationale = HIGH (`rule-authoring.md` Rule 10). `validate-xref-integrity` joined this set once it reached exit-0-on-clean at loom and was wired as a blocking gate — read its ABSOLUTE exit, never a diff against a baseline. It is NOT diff-relative: it scans the whole `.claude/{commands,rules,skills,agents,guides}` tree, and the carve-outs it prints for auditability (sanctioned Phase-2-deferred fixture refs, absent-by-design refs, directory-resolutions) are already excluded from `dangling` and do not drive the exit code — counting those printed lines as findings is the tally-not-hits error `instrument-discipline.md` MUST-3(b) names. The greening acceptance criteria are recorded at `journal/0182` § "Forest item — validate-xref-integrity greening" and are SATISFIED, not superseded.
...
- **`consumer-efficacy` — an ERRORED or MISSING result is UNKNOWN, never clean (RS-91).** The bare `validate-emit` above ALREADY runs this check: MEASURED, it is one of the 31 checks in the DEFAULT set, so no `--check consumer-efficacy` selector is needed (an earlier framing claimed the invocation was absent entirely — that premise is FALSE at loom; the invocation is line 26 and is already a hard gate). What the exit code does NOT tell you is whether the check's rows were PRODUCED. Read the rows, not the exit: `validate-emit --json` → the `consumer-efficacy` entry → confirm the `source:` / `installed:` coverage rows are present and their denominators are non-zero (`N of N SKILL.md parsed and asserted`). A check whose rows are ABSENT, or whose coverage denominator is 0, reports UNKNOWN — it MUST be re-run, and MUST NOT be recorded as a clean audit dimension (`instrument-discipline.md` MUST-1 + `evidence-first-claims.md` MUST-3: an errored or empty result is zero evidence, never confirmation). A SKIP row for an absent `.codex/` or `.gemini/` tree is a legitimate SKIP, not an UNKNOWN — the consumer has no such tree to assert over.
