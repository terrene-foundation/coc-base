---
id: "STACK-DETECTION"
paths: ["**/STACK.md", "**/.claude/agents/onboarding/**", "**/.claude/commands/onboard-stack.md", "**/.claude/skills/40-stack-onboarding/**"]
---
...
- **Detection mechanism:** two layers — a SHIPPED specialist-refusal gate and gate-review. **NO HOOK LAYER — this rule claims none.** (a) **SPECIALIST REFUSAL (SHIPPED, agent-invocation-time).** All three base-variant generic specialists read `STACK.md` as their first step and HALT on missing-or-low-confidence, each naming this rule by name: `.claude/agents/generic/ai-specialist.md:25-30`, `api-specialist.md:25-30`, `db-specialist.md:25-30` — line 25 is the read (`test -f STACK.md && cat STACK.md || echo "STACK.md missing"`), line 28 the halt condition (`confidence: LOW` / `UNKNOWN`), line 30 the emitted refusal citing "`rules/stack-detection.md` MUST-1, downstream `/implement` is BLOCKED". This layer CASCADES with the rule: all three ride the SAME `onboarding` tier as `rules/stack-detection.md` itself (`.claude/sync-manifest.yaml:1875-1877` vs `:1880`), so a consumer that pulled this rule also pulled the refusal. **SCOPE, stated so this does not become the previous over-claim in the other direction:** it fires ONLY when one of those three specialists is invoked. It is NOT a phase-command gate and NOT a corpus sweep — a phase command run without a specialist passes through it untouched. (b) **GATE-REVIEW (Phase 1, manual).** cc-architect at `/codify` + reviewer at `/implement` read any session that ran a phase command in a `STACK.md`-bearing project and confirm the stack was read from `STACK.md` rather than re-detected inline. Probes `.claude/test-harness/probes/stack-detection.probes.json` — NOT YET AUTHORED, declared in `phase2-deferrals.json::probe_authorship_deferrals`. Phase 2 (deferred per `trust-posture.md` § Two-Phase Rollout) — the mechanical sweep this bullet PREVIOUSLY asserted as shipped does not exist: `.claude/agents/cc-architect.md` contains no `STACK.md` instruction (0 occurrences) and none of the six phase command bodies (`analyze`, `todos`, `implement`, `redteam`, `codify`, `release`) mentions `STACK.md` (0 each), so "every phase command body greps for `STACK.md` and refuses without it" was true of zero commands. Wiring the sweep requires first landing the read-STACK.md-first step in those command bodies; audit fixtures land with that detector at `.claude/audit-fixtures/stack-detection/` per `cc-artifacts.md` Rule 9.
