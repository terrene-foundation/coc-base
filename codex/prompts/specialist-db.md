...
```bash
bin/coc <phase> "$(cat .codex/prompts/specialist-db.md)\n\nTask: <your task>"
```

Your context then contains the operating specification below. Read the task and respond as the db specialist.
...
Native subagent spawning is unreliable in headless mode. Use pattern (a): inline-cat `.codex/prompts/specialist-db.md` into the turn, then provide your task in the same session.
